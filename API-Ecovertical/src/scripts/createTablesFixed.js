...
async function createMissingTables() {
  ...
  try {
    ...
    // Configuración correcta basada en el diagnóstico
    ...
    const [tables1] = await connection.execute("SHOW TABLES LIKE 'alertas_programadas_riego'");
    if (tables1.length === 0) {
      ...
      await connection.execute(createTable1);
    ...
    const [tables2] = await connection.execute("SHOW TABLES LIKE 'alertas_riego'");
    if (tables2.length === 0) {
      ...
      await connection.execute(createTable2);
    ...
    const [tables3] = await connection.execute("SHOW TABLES LIKE 'notificaciones_alertas_riego'");
    if (tables3.length === 0) {
      ...
      await connection.execute(createTable3);
    ...
    const [tables4] = await connection.execute("SHOW TABLES LIKE 'notificaciones_alertas'");
    if (tables4.length === 0) {
      ...
      await connection.execute(createTable4);
    ...
    const [tables5] = await connection.execute("SHOW TABLES LIKE 'usuarios_conectados'");
    if (tables5.length === 0) {
      ...
      await connection.execute(createTable5);
      console.log('✅ Tabla usuarios_conectados creada');
    } else {
      console.log('✅ Tabla usuarios_conectados ya existe');
    }

    console.log('\n🎉 ¡Todas las tablas necesarias han sido creadas exitosamente!');
    console.log('✅ Los errores de "Table doesn\'t exist" deberían desaparecer ahora.');
